#!/usr/bin/env python3
"""attention_screening: single-command kinase-ligand interaction profiling.

Auto-detects input type and runs the appropriate inference pipeline:

  # Inline strings (no file required)
  python attention_screening.py "CC(=O)Oc1ccccc1C(=O)O"      → SMILES vs kinome
  python attention_screening.py "MGNNHGTYLG..."              → AA sequence vs ligand library

  # Files
  python attention_screening.py compound.smi                 → SMILES file
  python attention_screening.py protein.fa                   → FASTA file
  python attention_screening.py batch.csv                    → batch (auto-detects which
                                                             column is SMILES vs sequence)
  python attention_screening.py pairs.tsv                    → idem (TSV)
  python attention_screening.py pairs.txt                    → idem (auto-sniff delimiter)
...
where <run_id> = attention_screening_<timestamp>_<input_hash>

Usage examples:
  python attention_screening.py "CC(=O)Oc1ccccc1C(=O)O"        # H + NH (default)
  python attention_screening.py "CC(=O)Oc1ccccc1C(=O)O" --organism human
  python attention_screening.py "MATGEFSLIARYFDRVKSARLDVE..."  # vs ligand library
  python attention_screening.py my_protein.fa --top-k 20
  python attention_screening.py imatinib.smi --models dtkinase,drugban
  python attention_screening.py pairs.csv --out custom/path
"""
...
def detect_input(arg: str) -> tuple[str, dict]:
    ...
    raise SystemExit(
        ...
        f"\nTry one of:\n"
        f"  python attention_screening.py 'CC(=O)Oc1ccccc1C(=O)O'   # inline SMILES\n"
        f"  python attention_screening.py 'MGNNHGTYLG...'           # inline AA sequence\n"
        f"  python attention_screening.py protein.fa                # FASTA file\n"
        f"  python attention_screening.py compound.smi              # SMILES file\n"
        f"  python attention_screening.py pairs.csv                 # batch file\n"
    )
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="attention_screening: auto-detect input + run inference pipeline.",
    ...
    if args.out is None:
        ...
        args.out = REPO / "results" / "inference" / f"attention_screening_{ts}_{h}"
