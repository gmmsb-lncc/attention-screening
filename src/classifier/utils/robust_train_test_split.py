"""
Implementação robusta de divisão train/test para o DockTKinase.
...
class RobustTrainTestSplitter:
    """
    Divisão train/test robusta e cientificamente válida.
    ...
    def robust_split(
        ...
        """
        Realiza divisão train/test robusta com validação.
        ...
        Args:
            ...
            validate_split: Se deve validar estatisticamente a divisão
            
        Returns:
            X_train, X_test, y_train, y_test, validation_report
        """
        logger.info(f"🔄 Iniciando divisão robusta train/test ({test_size*100:.0f}% teste)")
        ...
        # 4. VALIDAÇÃO PÓS-DIVISÃO
        validation_report = self._validate_split(
            y_train, y_test, unique_classes, imbalance_ratio, issues
        ) if validate_split else self._create_minimal_report()
        ...
        if validation_report.issues:
            logger.warning("⚠️  Problemas na divisão:")
        ...
        return X_train, X_test, y_train, y_test, validation_report
    ...
    def _chi_square_test(self, train_counts: np.ndarray, test_counts: np.ndarray) -> float:
        ...
        # FIX #41: Especificar exceções esperadas ao invés de bare except
...
def robust_train_test_split(
    ...
    **kwargs
    ...
    """
    splitter = RobustTrainTestSplitter(random_state=random_state)
    return splitter.robust_split(X, y, test_size=test_size, **kwargs)
