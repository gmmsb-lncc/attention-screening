...
## 6.15. Validação empírica do pipeline de comitê via teste com imatinib (lição 25)

A primeira execução fim-a-fim do *committee orchestrator* sobre uma
molécula publicamente conhecida — o inibidor de tirosina-cinase
**imatinib** (CHEMBL941, mesilato de Gleevec; alvo primário ABL1) —
foi conduzida em `diamante-01` em 2026-05-06, integrando os três modelos
do *human\_kinome profile* (DT-Kinase + DrugBAN + ConPLex) sobre o
*kinome* humano de referência. A execução cumpria três objetivos
distintos: (i) verificar a operacionalidade do pipeline `expand\_pairs
→ \{dtkinase,drugban,conplex\}\_score → aggregate` em condições reais,
(ii) verificar se a triagem produzia ranking compatível com a
*ground truth* farmacológica da molécula, e (iii) identificar pontos
de regressão estrutural no pipeline de inferência. Cada um dos três
objetivos produziu um achado metodologicamente distinto, registrados
abaixo como subseções 6.15.1, 6.15.2 e 6.15.3.

### 6.15.1. *Bug* estrutural na calibração de Platt na inferência

A primeira execução com o checkpoint canônico do corpus humano
revelou que cada um dos cinco arquivos `level4\_cnn\_calibration.json`
contém parâmetros de Platt no estado-identidade ($a = 1{,}0$,
$b = 0{,}0$), com a seguinte nota textual:

> *"y\_prob in npz is post-Platt at training; identity Platt
> re-applied at inference"*

A nota documenta a hipótese de que o `raw\_predictions.npz` saved
durante o treinamento já continha probabilidades pós-Platt, dispensando
recalibração na inferência. **A hipótese é falsa**: a inspeção do
arquivo `npz` revelou distribuição de probabilidades com mediana
$0{,}21$ e amplitude $[0{,}0001,\, 0{,}9997]$, característica de saída
sigmoide pré-calibração e não de regressão logística calibrada.
Consequentemente, o caminho de inferência aplicava
$\sigma(\text{logit})$ cru com limiares MCC-óptimos calculados sobre
probabilidades efetivamente pós-Platt-treino, gerando
*decoupling* sistemático entre o limiar declarado e a escala em
que ele opera. A consequência empírica observada na inferência do
imatinib foi compressão da faixa de probabilidades para $[0{,}28,\,
0{,}42]$ com mediana $0{,}34$ — todos os 483 *kinases* praticamente
empatados em torno do limiar $0{,}405$.

A correção foi implementada em `scripts/inference/refit\_calibration\_offline.py`:
para cada $(corpus, seed)$ disponível, o script recupera os logits
via $\log(p/(1-p))$ a partir de `y\_prob`, ajusta uma regressão
logística de uma variável (logit-único) sobre $y\_true$, e
realiza a varredura de duas passagens (100 grosseiros + 100 finos)
para o limiar MCC-ótimo sobre as probabilidades pós-Platt-novo.
Os parâmetros recuperados para `human` foram $a \in [0{,}47,\, 0{,}56]$
e $b \in [-0{,}10,\, -0{,}01]$, distantes da identidade declarada e
indicando que **o classificador subjacente é sistematicamente
overconfident** (coeficiente $a < 1$ comprime o range das
probabilidades em direção a $0{,}5$). A re-execução do teste
do imatinib após a correção da calibração reduziu o $\sigma$
inter-semente do DT-Kinase em $26\%$ (mediana $0{,}197 \to 0{,}146$),
mas — como esperado — não alterou o *rank-order* das predições, já
que Platt é uma transformação monotônica.

Lição metodológica registrada (Lição 25-A): **a calibração
declarada como "identidade no inferência" deve ser tratada como
suspeita, não como otimização**. O monotonic-preservation property
de Platt esconde o erro: o ranking permanece visivelmente coerente
mesmo com calibração identidade, mas o limiar absoluto declarado nas
saídas é sistematicamente errado. A verificação rigorosa exige
inspeção cruzada entre $a, b, threshold$ declarados e a distribuição
empírica de `y\_prob` no `npz` correspondente.

### 6.15.2. Cobertura insuficiente do FASTA de referência

A segunda execução, agora com calibração corretamente aplicada,
permitiu identificar uma lacuna estrutural na biblioteca de quinases
de referência (`data/reference/kinome\_human.fasta`). O FASTA contém
$483$ sequências, indexadas por *component\_id* da tabela
`component\_sequences` do ChEMBL — herança do pipeline original de
construção do *dataset* (definido em
`src/database/sql/kinase\_humans.sql`). O contraste com o conjunto
de alvos farmacológicos canônicos do imatinib expõe a limitação:

| Alvo (UniProt) | Gene | Presente no FASTA legacy? |
|---|---|---|
| P00519 | ABL1 | sim |
| P42684 | ABL2 | sim |
| P06239 | LCK | sim |
| P10721 | KIT | **ausente** |
| P16234 | PDGFRA | **ausente** |
| P09619 | PDGFRB | **ausente** |
| P07333 | CSF1R | **ausente** |
| Q08345 | DDR1 | **ausente** |
| Q16832 | DDR2 | **ausente** |

Apenas três dos nove alvos canônicos do imatinib são candidatos
candidatos para a triagem. Os seis ausentes correspondem a
**RTKs tipo III** (KIT, PDGFRA/B, CSF1R, DDR1/2) — uma família
inteira de quinases ligadas à oncologia do imatinib que era
estruturalmente inacessível ao pipeline.

A causa do descasamento é que o FASTA foi construído pelo subconjunto
de quinases vistas no *training corpus* — apenas as quinases que
aparecem como alvos em ensaios ChEMBL incluídos no recorte da tese.
Não é o *kinome* humano canônico (≈ 530 sequências segundo Manning
et al. 2002), mas sim a *kinome-coverable-by-the-thesis-dataset*.

A correção foi implementada via *query* da API REST do UniProt sobre
*Swiss-Prot human reviewed* com filtragem por código EC (`2.7.10.*`,
`2.7.11.*`, `2.7.12.*`, `2.7.13.*`), produzindo $488$ sequências
indexadas por *accession* UniProt. O novo FASTA é mantido em paralelo
ao *legacy* (em `data/reference/kinome\_human\_uniprot.fasta`), e
`expand\_pairs.py` foi atualizado para preferir o novo *quando
presente*, com *fallback* para o legacy. A re-execução do teste
do imatinib sobre o FASTA novo confirmou a presença de **todos os
nove alvos canônicos** (verificável diretamente via *accession* na
saída).

Lição metodológica registrada (Lição 25-B): **a biblioteca de
sequências usada na inferência deve ser dissociada do dataset de
treinamento**. A inferência opera fora da distribuição de
treinamento por construção; restringir o universo de candidatos
aos alvos vistos no treinamento elimina, ainda na fase de
*expansão de pares*, qualquer hipótese sobre alvos ausentes do
*training corpus*. A separação requerida é principial, não
implementacional: o FASTA usado na inferência é parte do **espaço
de busca do problema clínico/farmacológico**, não parte do
dataset experimental.

### 6.15.3. Diferenciação por arquitetura sob *out-of-training-kinome*

A presença dos seis novos alvos no FASTA permitiu observar
empiricamente como cada arquitetura responde a quinases
**ausentes do seu corpus de treinamento**. O contraste é
metodologicamente revelador (corpus humano, comitê 3-modelo,
agregação por média aritmética):

| Alvo | Rank comitê | DT-Kinase | DrugBAN | ConPLex |
|---|---:|---:|---:|---:|
| ABL1 (in-corpus) | 1 | $0{,}39$ | $0{,}83$ | $0{,}89$ |
| ABL2 (in-corpus) | 2 | $0{,}38$ | $0{,}51$ | $0{,}87$ |
| LCK (in-corpus) | 5 | $0{,}37$ | $0{,}62$ | $0{,}71$ |
| PDGFRA (out-of-corpus) | 41 | $0{,}37$ | $0{,}12$ | $0{,}71$ |
| PDGFRB (out-of-corpus) | 72 | $0{,}37$ | $0{,}07$ | $0{,}70$ |
| KIT (out-of-corpus) | 84 | $0{,}37$ | $0{,}05$ | $0{,}72$ |
| CSF1R (out-of-corpus) | 91 | $0{,}38$ | $0{,}04$ | $0{,}71$ |
| DDR1 (out-of-corpus) | 103 | $0{,}38$ | $0{,}06$ | $0{,}67$ |
| DDR2 (out-of-corpus) | 129 | $0{,}36$ | $0{,}03$ | $0{,}70$ |

A leitura por coluna revela três regimes distintos:

- **DT-Kinase** produz probabilidades praticamente uniformes
  ($\sim 0{,}37$) tanto em alvos *in-corpus* quanto *out-of-corpus*
  — sintoma da fragilidade já documentada nas Lições 21 e 22 sobre
  generalização *cross-scaffold*. O sinal aparece como ruído
  inter-semente ($\sigma \sim 0{,}14$) e o ranking individual do
  modelo isolado é essencialmente aleatório.
- **DrugBAN**, treinado *from-scratch* (CNN1D + GCN, sem PLM
  pré-treinado), exibe **colapso de confiança** sobre alvos
  *out-of-corpus*: probabilidades $\sim 0{,}05$ para os seis
  RTKs ausentes do *training corpus*, contra $\sim 0{,}7$ para os
  três presentes. O comportamento é consistente com o regime
  documentado na matriz cross-dataset 3×3 (Anexo A da tese): MCC
  alto in-domain, mas com sensibilidade severa à composição do
  *training corpus* — agora demonstrado por inspeção direta de
  predições, não apenas por métrica agregada.
- **ConPLex**, que usa ProtBert (PLM proteico genérico) + Morgan
  FP, generaliza para os seis alvos *out-of-corpus* com
  probabilidades $\sim 0{,}7$, comparáveis aos *in-corpus*. A
  hipótese mecanística é que o ProtBert representa kinase-domain
  por similaridade global ao kinome humano, não por similaridade
  específica ao kinome-treino — fornecendo invariância parcial
  ao corpus de treinamento.

A consequência operacional sobre o comitê é que os seis alvos
*out-of-corpus* recebem voto positivo apenas do ConPLex (1 dos 3
modelos), caindo no *tier* `UNCERTAIN` mesmo que ConPLex
isoladamente classificasse-os como ligantes. O *tier* `LIKELY`
exige $\geq 2$ votos positivos sob a regra atual, configurando o
caso paradigmático em que **a regra de agregação por contagem de
votos descarta sinais corretos quando a maioria dos modelos
falha por razões correlacionadas** (DrugBAN e DT-Kinase
compartilham o mesmo *training corpus*, portanto falham juntos
fora dele).

Lição metodológica registrada (Lição 25-C): **a regra de tier
por contagem de votos é vulnerável a falhas correlacionadas
entre modelos do comitê**. Modelos que compartilham *training
corpus* tendem a falhar em conjunto sobre alvos
*out-of-training*, o que pode levar à classificação `UNCERTAIN`
de alvos verdadeiros mesmo quando uma arquitetura discordante
(ConPLex, com PLM genérico) os identifica corretamente. A
mitigação principial requer ou (i) ponderação por confiabilidade
arquitetural a priori — ConPLex receberia peso maior em
RTKs *out-of-thesis-corpus* — ou (ii) substituir contagem de
votos por meta-classificador treinado sobre as probabilidades
calibradas dos modelos individuais, capturando dependências
não-lineares entre os votos. O Apêndice B ($\S$B.3) propõe
formalmente o segundo caminho.

### 6.15.4. Contribuição do teste do imatinib à narrativa do comitê

O teste do imatinib é, simultaneamente, uma execução de validação
e uma execução de descoberta. A função de validação é direta:
ABL1 emerge no posto 1 do *ranking* do comitê 3-modelo, sob critério
`LIKELY`; o resultado é metodologicamente correto sob o protocolo
canônico e empiricamente compatível com a *ground truth*
farmacológica do imatinib. A função de descoberta opera em três
camadas distintas, cada uma identificando uma falha estrutural do
pipeline de inferência (calibração de Platt em estado-identidade,
cobertura insuficiente do FASTA de referência, regra de tier
vulnerável a falhas correlacionadas) que não eram observáveis nos
*benchmarks* multi-semente sobre o conjunto de teste do *training
corpus*. As três falhas são distintas em natureza — *bug* de
calibração, lacuna de cobertura, escolha de agregação — mas
compartilham um traço comum: nenhuma delas é detectável por
métrica agregada in-domain. **Apenas a inferência sobre uma
molécula com *ground truth* externa pode revelá-las**.

A consequência metodológica é que, ao lado do protocolo canônico
de validação multi-semente sobre o *test split* do *training
corpus*, **o pipeline de inferência exige um conjunto de testes
de regressão baseados em moléculas com perfil farmacológico
conhecido**. O imatinib é o primeiro teste desse tipo registrado;
tipos III RTK (KIT, PDGFRA/B) constituem a primeira família
para a qual o comitê atual é estruturalmente inadequado e que
exige melhorias arquiteturais (4º modelo com PLM genérico,
ponderação por confiança, ou meta-classificador). Outros testes
de regressão sugeridos para a próxima iteração: gefitinib (alvo
EGFR P00533), dasatinib (multi-quinase, BCR-ABL + Src),
crizotinib (ALK + ROS1), ruxolitinib (JAK1/JAK2). Cada um
estressará uma região distinta do espaço-de-quinases e revelará
falhas potencialmente novas.
