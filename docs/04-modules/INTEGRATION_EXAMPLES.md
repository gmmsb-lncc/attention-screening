...
This document provides complete, copy-paste ready examples for integrating new protein models (ESM-3, OpenFold, ProtTrans, etc.) into the docktkinase protein embedding system.

## Table of Contents

1. [ESM-3 Integration](#esm-3-integration)
2. [OpenFold Integration](#openfold-integration)
3. [ProtTrans Integration](#prottrans-integration)
4. [Custom Model Integration](#custom-model-integration)
...
### Step 1: Create OpenFold Strategy

Create file: `/src/build/embeddings/strategies/openfold_strategy.py`

```python
"""
OpenFold strategy implementation.

OpenFold is an open-source implementation of AlphaFold2.
Can extract embeddings from structure prediction pipeline.
"""
...
class OpenFoldStrategy(BaseProteinStrategy):
    """
    Strategy implementation for OpenFold models.
    
    Note: OpenFold typically requires MSA (Multiple Sequence Alignment).
    This implementation provides sequence-only embeddings for simplicity.
    """
    
    MODEL_SPECS = {
        'openfold_model_1': {'dim': 384, 'max_len': 2048},
        'openfold_model_2': {'dim': 384, 'max_len': 2048},
    }
    
    def __init__(self):
        """Initialize OpenFold strategy."""
        self.logger = None
        self._cache_dir = None
    ...
    def load(
        ...
        **kwargs
        ...
        if model_name not in self.MODEL_SPECS:
            raise ValueError(f"OpenFold model '{model_name}' not supported")
        
        try:
            if self.logger:
                self.logger.info(f"Loading OpenFold model: {model_name}")
            ...
            try:
                ...
                raise ModelLoadError(
                    "OpenFold not available. Install from: "
            ...
            if self.logger:
                self.logger.info(f"✅ OpenFold model loaded")
            
            return model, {'config': config}
            
        except Exception as e:
            raise ModelLoadError(f"Failed to load OpenFold: {e}")
    ...
    def generate(
        ...
        **kwargs
        ...
        """
        Generate OpenFold embedding.
        
        Note: This extracts embeddings from single sequence representation.
        For full structure prediction, use OpenFold's complete pipeline.
        """
        config = auxiliary_objects['config']
        self.logger = kwargs.get('logger', self.logger)
        ...
        if not clean_sequence:
            raise EmbeddingError("Invalid sequence")
        ...
        try:
            # Prepare input (simplified - actual OpenFold needs more)
            # This is example code - adjust to actual OpenFold API
            from openfold.data import data_transforms
            
            # Convert sequence to features
            features = self._prepare_features(clean_sequence, device)
            
            # Forward pass
            with torch.no_grad():
                outputs = model(features)
                
                # Extract single representation embedding
                # Adjust key based on actual OpenFold output
                embedding = outputs['single'].mean(dim=1).squeeze()
                result = embedding.cpu().numpy()
            
            # Cleanup
            del features, outputs, embedding
            gc.collect()
            if device.type == 'cuda':
                torch.cuda.empty_cache()
            ...
            if device.type == 'cuda':
                torch.cuda.empty_cache()
            raise EmbeddingError(f"OpenFold embedding failed: {e}")
    ...
    def get_embedding_dim(self, model_name: str) -> int:
        ...
        return self.MODEL_SPECS.get(model_name, {}).get('dim', 384)
    ...
    def _prepare_features(self, sequence: str, device: torch.device) -> Dict:
        """
        Prepare input features for OpenFold.
        
        Note: This is simplified. Full OpenFold requires:
        - MSA generation
        - Template search
        - Feature processing pipeline
        """
        # Placeholder - implement actual feature preparation
        # See OpenFold documentation for complete implementation
        raise NotImplementedError(
            "OpenFold feature preparation not implemented. "
            "See: https://github.com/aqlaboratory/openfold/tree/main/openfold/data"
        )
```
