    ...
    def run_single_seed(
        ...
        **kwargs: object,
        ...
        if os.path.exists(cache_path) and not self.force and strict_freeze:
            tqdm.write("  Strict test mode: ignoring cached Level 2 results and recomputing.")
        ...
        if self.mode == "test":
            frozen_selection = _load_frozen_mlp_selection_from_train(
                ...
                cache_filename="level2_knn_mlp_results.json",
            )
            if strict_freeze and frozen_selection is None:
                raise RuntimeError(
                    "Missing frozen train selection for Level 2 test run. "
