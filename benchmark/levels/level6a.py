"""Level 6a — CrossAttn + BAN + GRL + KNN/MLP.

Replaces the AttnPool + concat + classifier of Level 5a with Bilinear
Attention Network (BAN) fusion.  Cross-attention still enriches per-token
representations before BAN performs joint interaction modeling + pooling.

Pipeline:
  1.  Cluster training scaffolds (same as L5).
  2.  Train Level6aModel via ``run_single_analysis(model_variant="level6a")``.
...
class Level6aRunner(BaseLevelRunner):
    """CrossAttn + BAN + GRL → canonical KNN/MLP."""
    ...
    def _extract_features(
        ...
        from crossattention_split_analysis.models.level6a import Level6aModel
        ...
        prefix = f"{self.dataset}_ban6a_molformer_{short_emb}_seed{seed}_"
    ...
    def _extract_mlp_metrics(self, results: Dict) -> Dict[str, float]:
        ...
        for nested_key in ("Level6a", "level6a"):
