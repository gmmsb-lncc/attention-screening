...
representations invariant to scaffold membership, improving
generalisation on the scaffold-split test set.

The only architectural difference from Level 4 is:
  - A ``GradientReversalLayer`` + ``DomainDiscriminator`` branch
    added after the pooled ``combined`` vector.
  - Scaffold-cluster domain labels injected into the training loop.

Pipeline:
  1.  Cluster training scaffolds into K groups (Butina / Tanimoto).
  2.  Train Level5DAModel (= Level5LiteModel backbone + GRL) via
      ``run_single_analysis`` with ``model_variant="level5_da"``.
...
class Level5Runner(BaseLevelRunner):
    """Cross-Attention + GRL domain adaptation → canonical KNN/MLP."""
    ...
    def run_single_seed(
        ...
        **kwargs: object,
        ...
        """Train DA model, extract scaffold-invariant features, run KNN/MLP."""
        from crossattention_split_analysis.experiment import run_single_analysis

        os.makedirs(output_dir, exist_ok=True)

        cache_path = os.path.join(output_dir, "level5_knn_mlp_results.json")
        strict_freeze = (
            self.mode == "test"
        ...
        if os.path.exists(cache_path) and not self.force and not strict_freeze:
            tqdm.write(f"  Loading cached Level 5 results (seed {seed})")
            with open(cache_path) as fh:
                return json.load(fh)
        if os.path.exists(cache_path) and not self.force and strict_freeze:
            tqdm.write("  Strict test mode: ignoring cached Level 5 results and recomputing.")

        # --- Step 1: Train Level 5 DA model ---
        tqdm.write(f"  Training Level 5 DA encoder (seed {seed})...")
        ...
        try:
            ...
            if strict_freeze:
                raise RuntimeError(
                    "Level 5 strict test mode forbids fallback metrics on feature extraction failure."
                ) from exc
        ...
        if self.mode == "test":
            frozen_selection = _load_frozen_mlp_selection_from_train(
                output_dir=output_dir,
                cache_filename="level5_knn_mlp_results.json",
            )
            if strict_freeze and frozen_selection is None:
                raise RuntimeError(
                    "Missing frozen train selection for Level 5 test run. "
    ...
    def _extract_features(
        ...
        """Load DA checkpoint, extract pre-head features."""
        from crossattention_split_analysis.config import (
            SUPPORTED_EMBEDDINGS as CA_SUPPORTED_EMBEDDINGS,
        )
        from crossattention_split_analysis.models.level5_da import Level5DAModel
        ...
        prefix = f"{self.dataset}_da_molformer_{short_emb}_seed{seed}_"
