    ...
    def run_single_seed(
        ...
        **kwargs: object,
        ...
        if os.path.exists(cache_path) and not self.force and strict_freeze:
            tqdm.write("  Strict test mode: ignoring cached Level 6b results and recomputing.")
        ...
        try:
            ...
            if strict_freeze:
                raise RuntimeError(
                    "Level 6b strict test mode forbids fallback metrics on feature extraction failure."
                ) from exc
        ...
        if self.mode == "test":
            frozen_selection = _load_frozen_mlp_selection_from_train(
                output_dir=output_dir,
                cache_filename="level6b_knn_mlp_results.json",
            )
            if strict_freeze and frozen_selection is None:
                raise RuntimeError(
                    "Missing frozen train selection for Level 6b test run. "
