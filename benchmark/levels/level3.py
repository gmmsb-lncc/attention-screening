"""Level 3 — Bimodal attention pooling + KNN/MLP.

Loads the **same** per-residue ESM-2 protein matrices and per-token
MoLFormer ligand matrices used by Level 2, but applies a **learned
attention pooling** mechanism (identical to Level 1c's backbone) to
produce fixed-size vectors for each modality independently.

Architecture (same backbone as Level 1c, applied to both modalities):
  1. Linear projection  (protein_dim → hidden_dim / ligand_dim → hidden_dim)
  2. Attention pooling   (learnable query, single-query multi-head attention)
  3. Concatenation       [protein_vec ‖ ligand_vec]  →  [B, 2 × hidden_dim]

Difference from other levels:
  - Level 1b: mean pooling (no parameters), ligand only
  - Level 1c: projection + attention pooling, ligand only
  - Level 2: mean pooling (no parameters), bimodal
  - **Level 3: projection + attention pooling, bimodal**
  - Level 4: CNN 2D on interaction maps

This level isolates the contribution of *learned aggregation* applied
to both modalities, using the exact same projection and pooling
architecture as Level 1c.
...
# ---------------------------------------------------------------------------
# Attention-pooling model (same backbone as Level 1c, bimodal)
# ---------------------------------------------------------------------------

class _AttentionPool(nn.Module):
    """Learnable-query attention pooling (Set Transformer style).

    Identical to the one used by Level 1c — single query vector,
...
class _BimodalAttentionPoolingModel(nn.Module):
    """Projection + attention pooling for protein–ligand pairs.

    Uses the **exact same** backbone as Level 1c's
    ``_LigandAttentionPoolingModel``, applied independently to both
    protein and ligand modalities:
      - 1-layer projection: Linear → LayerNorm → GELU → Dropout
      - Single-query attention pooling

    Output: concatenation [protein_vec ‖ ligand_vec] of shape [B, 2*hidden_dim].
    """
    ...
    def __init__(
        ...
        # Same 1-layer projection as Level 1c
        ...
        # Same single-query attention pooling as Level 1c
    ...
    def forward(
        ...
        """Return concatenated [protein_vec ‖ ligand_vec] of shape [B, 2*hidden_dim]."""
        ...
        return torch.cat([protein_vec, ligand_vec], dim=-1)
...
def _train_attention_pooling(
    ...
    patience: int | None = 5,
    seed: int = 42,
) -> _BimodalAttentionPoolingModel:
    """Train bimodal projection + attention pooling.
    ...
    Same training protocol as Level 1c.
    """
    ...
    model = _BimodalAttentionPoolingModel(
    ...
    # Simple auxiliary classification head (discarded after training)
    # Same as Level 1c: just nn.Linear(output_dim, 1)
...
def _extract_features(
    ...
    """Extract attention-pooled bimodal features."""
    model.eval()
...
class Level3Runner(BaseLevelRunner):
    """Bimodal attention pooling → KNN/MLP."""
    ...
    def run_single_seed(
        ...
        **kwargs: object,
        ...
        """Train bimodal attention pooling, extract features, run KNN/MLP."""
        ...
        tqdm.write(f"  Building Level 3 bimodal attention pooling (seed {seed})...")
        ...
        # Train bimodal projection + attention pooling
        tqdm.write("  Training bimodal projection + attention pooling...")
        ...
        if self.mode == "train":
            tqdm.write("  Extracting bimodal attention-pooled features (held-out train + val)...")
            x_fit, y_fit = _extract_features(model, feat_extract_loader, device)
            x_eval, y_eval = _extract_features(model, val_loader, device)
        else:
            tqdm.write("  Extracting bimodal attention-pooled features (val + test)...")
...
class Level3aRunner(Level3Runner):
    ...
    def run_single_seed(
        ...
        **kwargs: object,
        ...
        """Train bimodal attention pooling, extract features, run MLP only."""
        ...
        tqdm.write(f"  Building Level 3a bimodal attention pooling (seed {seed})...")
        ...
        tqdm.write("  Training MLP only (canonical classifier)...")
        frozen_selection = None
        if self.mode == "test":
            frozen_selection = _load_frozen_mlp_selection_from_train(
                output_dir=output_dir,
        ...
        tqdm.write(
            f"  Level 3a (seed {seed}): "
            f"MLP MCC={mlp_metrics['mcc']:.4f}"
        )
        return result
